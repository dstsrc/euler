...
func main() {
	...
	main := getMainMax()
	second := getSecondMax()
	fmt.Println(mr, mc, main, second)
}

func getSecondMax() int {
	max := 0
	for i := 3; i < 20; i++ {
		for j := 0; j <= 16; j++ {
			s := tbl[i][j] * tbl[i-1][j+1] * tbl[i-2][j+2] * tbl[i-3][j+3]
			if s > max {
				max = s
			}
		}
...
func getRowMax() int {
	...
	for i := 0; i < 20; i++ {
		for j := 0; j <= 16; j++ {
			s := tbl[i][j] * tbl[i][j+1] * tbl[i][j+2] * tbl[i][j+3]
			if s > max {
				max = s
			}
		}
...
func getRowCol() int {
	...
	for i := 0; i <= 16; i++ {
		for j := 0; j < 20; j++ {
			s := tbl[i][j] * tbl[i+1][j] * tbl[i+2][j] * tbl[i+3][j]
			if s > max {
				max = s
			}
		}
